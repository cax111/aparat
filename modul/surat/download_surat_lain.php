...
$tipe_download=$_POST['tipe_download'];
...
//format yang akan di proses menjadi dokumen word .docx
	if($tipe_download=="kak"){
		include 'surat_lain/kerangka_acuan_kerja/kerangka_acuan_kerja_docx.php';

		// Save file / download file
		ob_end_clean();
		header('Content-Type: application/octet-stream');
		header('Content-Disposition: attachment;filename=3.Kerangka Acuan Kerja '.$judul.'-'.$tahun.'.docx');
		header('Cache-Control: max-age=0');
		$objWriter = \PhpOffice\PhpWord\IOFactory::createWriter($phpWord, 'Word2007');
		$objWriter->save('php://output');

	}elseif($tipe_download=="form_kualifikasi"){
		include 'surat_lain/surat_form/surat_form_docx.php';

		// Save file / download file
		ob_end_clean();
		header('Content-Type: application/octet-stream');
		header('Content-Disposition: attachment;filename=4.Form Kualifikasi '.$judul.'-'.$tahun.'.docx');
		header('Cache-Control: max-age=0');
		$objWriter = \PhpOffice\PhpWord\IOFactory::createWriter($phpWord, 'Word2007');
		$objWriter->save('php://output');
	
	}elseif($tipe_download=="uraian_tanggal_kegiatan"){
		include 'surat_lain/surat_uraian/surat_uraian_docx.php';

		// Save file / download file
		ob_end_clean();
		header('Content-Type: application/octet-stream');
		header('Content-Disposition: attachment;filename=5.Uraian Tanggal Kegiatan '.$judul.'-'.$tahun.'.docx');
		header('Cache-Control: max-age=0');
		$objWriter = \PhpOffice\PhpWord\IOFactory::createWriter($phpWord, 'Word2007');
		$objWriter->save('php://output');
